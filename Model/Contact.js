...
class Contact {
  ...
  static async create({ name, age, email, phone, password }) {
    console.log('Received data model:', { name, age, email, phone, password });

    const result = await pool.query('INSERT INTO message (name, age, email, phone, password) VALUES ($1, $2, $3, $4, $5) RETURNING *',[name, age, email, phone, password]);
    return result.rows[0];
  }
